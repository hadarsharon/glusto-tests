...
class TestVolumeCreate(GlusterBaseClass):
    ...
    def test_add_brick_functionality(self):
        ...
        if not setup_volume(self.mnode, self.all_servers_info, self.volume):
            raise ExecutionError("Failed to create and start volume %s",
                                 self.volname)
        ...
        # of bricks
        if add_brick(self.mnode, self.volname, bricks_list[0])[0] == 0:
            raise ExecutionError("Expected: It should fail to add a single"
        ...
        if add_brick(self.mnode, self.volname, bricks_to_add, **kwargs)[0] == 0:
            raise ExecutionError("Expected: It should fail to add a non "
                                 "existing brick to a volume. Actual: "
                                 "Successfully added a non existing brick "
                                 "to volume")
        ...
        if add_brick(self.mnode, self.volname, bricks_to_add, **kwargs)[0] == 0:
            raise ExecutionError("Expected: It should fail to add brick from "
                                 "a node which is not part of a cluster."
                                 "Actual:Successfully added bricks from node "
        ...
        if add_brick(self.mnode, self.volname, bricks_to_add, **kwargs)[0] != 0:
            raise ExecutionError("Failed to add the bricks to the volume")
        g.log.info("Successfully added bricks to volume")

        # Perform rebalance start operation
        if rebalance_start(self.mnode, self.volname)[0] != 0:
            raise ExecutionError("Rebalance start failed")
